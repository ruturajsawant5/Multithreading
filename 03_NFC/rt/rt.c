				      ...
				      char mask, char *gw_ip, char *oif)
{
	...
	if(gw_ip) strncpy(rt_entry->gw_ip, gw_ip, sizeof(rt_entry->gw_ip));
	if(oif) strncpy(rt_entry->oif, oif, sizeof(rt_entry->oif));
...
void rt_dump_rt_table(rt_table_t *rt_table)
{
	...
	ITERTAE_RT_TABLE_BEGIN(rt_table, rt_entry)
	{
		printf("%-20s %-4d %-20s %s\n", rt_entry->rt_entry_keys.dest,
		       rt_entry->rt_entry_keys.mask, rt_entry->gw_ip,
		       rt_entry->oif);
					...
					nfc_app_cb app_cb, uint32_t subs_id)
{
	...
	if(!rt_entry) {
		 ...
		 * */
		rt_entry = rt_add_or_update_rt_entry(rt_table, key->dest,
						     key->mask, 0, 0);
