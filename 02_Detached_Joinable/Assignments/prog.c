...
I wish life was so simple. You need to print the values from N to 1 ( in reverse
order) via multi-threading.

Your program should create N threads with thread id from 1 to N.  The Nth thread
must print N, the (N-1)th thread must print N-1 and so on .
*/
...
void *thread_function(void *arg)
{
	...
	// If this thread's index is less than N, create a child thread with
	// index+1.
	if(index < N) {
		...
		if(next_index == NULL) {
			fprintf(stderr,
				"Memory allocation failed for thread %d\n",
				index + 1);
			exit(EXIT_FAILURE);
		}
		*next_index = index + 1;
		...
		int rc = pthread_create(&child_thread, NULL, thread_function,
					next_index);
		if(rc != 0) {
...
int main(void)
{
	...
	if(first_arg == NULL) {
		fprintf(stderr,
			"Memory allocation failed for thread 1 argument\n");
		return EXIT_FAILURE;
	}
